...
x.pop('MasVnrArea')
x.pop('GarageYrBlt')
kernel_choice = 'sigmoid' #linear, rbf ,sigmoid
...
svmode_model.fit(x_tr, y_tr)
y_pred = svmode_model.predict(x_t)
...
corr =  houses_copy[['TotalBsmtSF','TotRmsAbvGrd', 'Clasificacion']].corr()
...
y_pred_train = svmode_model.predict(x_train_reg.values)
...
y_pred = svmode_model.predict(x_test_reg.values)
